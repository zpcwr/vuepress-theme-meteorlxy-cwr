...
export default {
  ...
  data () {
    return {
      ...
      scrollListener: throttle(() => {
        this.fixed = this.infoCardDom.getBoundingClientRect().bottom < this.headerHeight
      }, 100),
  ...
  computed: {
    style () {
      return {
        ...
        'top': this.fixed ? `${this.headerHeight}px` : 0,
        'width': `${this.width}`,
      }
    },
  ...
  methods: {
    getWidth () {
      let navCardEle = document.querySelector('.post-nav-card.main-div')
      if (this.infoCardDom && navCardEle){
        let width = this.infoCardDom.clientWidth
        width = width === 0 ? navCardEle.clientWidth : width
        return width + 'px'
      }
      return '0px'
    },
